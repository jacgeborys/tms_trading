...
def main():
    ...
    lines = [
        ...
        f"Balance     : {config.BALANCE:,.0f} PLN",
        f"Equity (T0) : {config.EQUITY:,.0f} PLN",
        f"Margin used : {config.MARGIN_USED:,.0f} PLN",
        "",
        ...
        "",
        f"── Pending orders ──────────────────────────────────",
        f"  Avg fills by day 90 :  {avg_fills:.1f} / {len(config.PENDING_ORDERS)}",
    ...
    for i, (lots, price) in enumerate(config.PENDING_ORDERS):
