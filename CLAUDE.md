...
| Rollover costs: March −9.52, June −12.53, Sep −12.52/−12.79 PLN / 0.001 lot | Back-calculated from survivorship grouping of open positions by rollover count. Sep 2026: two rates observed (−12.52 for Sep 11 positions, −12.79 for Sep 14+ positions) | Projected December cost ~−13 PLN / 0.001 lot at current rates |
...
| MT5 server time is UTC+2 (Polish CEST), not UTC | `mt5.symbol_info_tick().time` runs +7200s ahead of `time.time()`. Position timestamps, bar timestamps are all UTC+2 | When converting MT5 times, subtract 2 hours to get true UTC. Previous references to "UTC" in bar times were actually MT5 server time |
| Daily session break: 22:58 → ~00:00 MT5 time (~62 min) | M1 data shows last bar at 22:58, first bar at 00:00 (both MT5 server time = Polish time) | You're unavoidably unhedged for ~62 min during the break |
